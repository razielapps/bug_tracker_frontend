// components/ProjectCard.tsx
import { Card } from "@/components/ui/card"; // shadcn/ui
import { Briefcase, CheckCircle, CircleDot } from "lucide-react";
import Link from "next/link";
export default function ProjectCard({ issue }) {
    return (
        <Link href={`dashbaord/bugs/1`}>
        <Card className="p-4 shadow-sm rounded-2xl space-y-2">
            <div className="text-xl font-semibold flex items-center gap-2">
                <Briefcase className="w-5 h-5" />
                {issue.name}
            </div>
            <p className="text-sm text-muted-foreground">{issue.description}</p>
            <div className="flex items-center justify-between text-sm">
                <div>
                    <span className="text-blue-600">{issue.issues_count}</span> issues
                </div>
                <div className="flex gap-3">
                    <div className="flex items-center gap-1">
                        <CircleDot className="w-4 h-4 text-yellow-500" />
                        {issue.open_issues}
                    </div>
                    <div className="flex items-center gap-1">
                        <CheckCircle className="w-4 h-4 text-green-500" />
                        {issue.closed_issues}
                    </div>
                </div>
            </div>
            <div className="text-xs text-muted-foreground">
                Created by {issue.created_by.username} on{" "}
                {new Date(issue.created_at).toLocaleDateString()}
            </div>
        </Card>
        </Link>
    );
}
